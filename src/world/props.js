...
export function registerProps(A, rngIn) {
  ...
  /**
   * SOLID. The prototype carries its own collision proxy, generated by
   * `Assembler.place()` from the same matrix that draws the instance — see the
   * `collide` field in src/world/builder.js for why this is declared here and
   * not typed out again next to every placement. `tools/solidcheck.mjs` is the
   * gate: anything a player can stand against and cannot walk through.
   *
   * A prototype under `STANCE.stand.stepHeight` (0.42 m) is deliberately NOT
   * marked — a bucket, a cinder block, a jerry can and a lying tyre are all
   * stepped over by the controller without the player feeling anything, and a
   * proxy there would be triangles nobody can touch plus a bump in the bot
   * height field (`src/ai/nav.js` samples one ray per cell and cannot tell a
   * 0.2 m block from a 0.2 m step). `_protoBox` drops them on its own if one
   * is ever marked by mistake.
   */
  const SOLID = { collide: true };

  // containers
  P('crate_a', 'wood_prop', crate(rng, 0.64), { skirt: 0.37, ...SOLID, ...LOOSE(0.09, 0.022) });
  P('crate_b', 'wood_prop', crate(rng, 0.48), { ...SOLID, ...LOOSE(0.10, 0.018) });
  P('crate_c', 'wood_prop_dark', crate(rng, 0.82), { skirt: 0.45, ...SOLID, ...LOOSE(0.075, 0.026) });
  P('crate_flat', 'wood_prop', crate(rng, 0.55, false), { ...SOLID, ...LOOSE(0.10, 0.02) });
  P('box_card_a', 'wood_pale', cardboardBox(rng, 0.46), { ...SOLID, ...LOOSE(0.10, 0.016) });
  P('box_card_b', 'wood_pale', cardboardBox(rng, 0.34), LOOSE(0.11, 0.012));
  P('barrel_rust', 'metal_rust_prop', barrel(rng), { skirt: 0.28, ...SOLID, ...LOOSE(0.085, 0.014) });
  P('barrel_blue', 'metal_blue', barrel(rng, 0.28, 0.9, 2), { skirt: 0.26, ...SOLID, ...LOOSE(0.085, 0.014) });
  P('barrel_wood', 'wood_prop_dark', barrel(rng, 0.31, 0.78, 4), { skirt: 0.28, ...SOLID, ...LOOSE(0.09, 0.015) });
  P('gas_bottle', 'metal_green', gasBottle(rng), { skirt: 0.18, ...SOLID, ...LOOSE(0.07, 0.008) });
  ...
  /**
   * A chair is solid only up to the seat. Boxing the back as well turns a
   * 0.45 m thing you walk round into a 0.9 m thing you cannot see over, and
   * `floorcheck` floods interiors through the gaps between furniture.
   */
  P('chair', 'wood_prop', chair(rng), { collide: { h: 0.48 }, ...LOOSE(0.05, 0.012) });
  P('cabinet', 'wood_prop_dark', cabinet(rng), { skirt: 0.42, ...SOLID });

  // services
  P('ac_unit', 'metal_dark', acUnit(rng), SOLID);
  P('sat_dish', 'metal_dark', satDish(rng), SOLID);
  P('water_tank', 'metal_blue', waterTank(rng), { skirt: 0.48, ...SOLID });
  P('roof_vent', 'metal_rust', roofVent(rng), SOLID);
  ...
  P('planter', 'concrete_prop', planter(rng), { skirt: 0.33, ...SOLID, ...LOOSE(0.07, 0.014) });
